...
module.exports = {
  ...
  newTransaction: async (req, res) => {
    ...
    const dataSpecificTechnicalActivity = data.filter(
      (el) => el.type === "specific technical activity"
    );
    ...
    if (dataOffer[0]) {
      ...
      try {
        const newActivites = await SpecificTechnicalActivity.bulkCreate(
          newSpecificTechnicalActivities
        );
        ...
        await ClientNeed.bulkCreate(updateClientNeeds, {
          updateOnDuplicate: [
            "status",
            "SpecificTechnicalActivityId",
            "location",
          ],
        });
      } catch (error) {
